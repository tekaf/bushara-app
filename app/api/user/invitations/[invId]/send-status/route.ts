...
export async function GET(request: NextRequest, { params }: { params: { invId: string } }) {
  try {
    ...
    for (const doc of guestsSnap.docs) {
      ...
      const attempts = Number(row?.sendAttemptCount || 0)
    ...
    const recentJobs = recentJobsSnap.docs
      .map((d) => ({ id: d.id, ...(d.data() as any) }))
      .sort((a, b) => {
        const aT = new Date(toIso(a.createdAt) || 0).getTime()
        const bT = new Date(toIso(b.createdAt) || 0).getTime()
        return bT - aT
      })
      .slice(0, 10)
      .map((j) => ({
        ...
        createdAt: toIso(j.createdAt),
        updatedAt: toIso(j.updatedAt),
        processedAt: toIso(j.processedAt),
      }))
    ...
    const summary = {
      ...
      lastSendAt: toIso(invite?.lastSendAt) || lastLogAt,
    }
    ...
    return NextResponse.json({
      ...
      invitation: {
        ...
        sendStatusSummary: invite?.sendStatusSummary || null,
      },
      summary,
      breakdown: {
      ...
      source: {
        summaryComputedFrom: 'guests',
        breakdownFrom: ['guests', 'send_logs'],
      },
    })
  } catch (error: any) {
